...
HardwareSerial controllerData(2); // Use UART2 (GPIO16 and GPIO17 for TX and RX) for ESP32 DEVKIT v1

#define controllerDataBaud 9600
#define rxd2 16
#define txd2 17
...
void dumpGamepad(ControllerPtr ctl) {
    ...
    controllerData.println();
    
    delay(50);
       
    Serial.println("SENT CONTORLLER ON SERIAL1");
...
void processGamepad(ControllerPtr ctl) {
    ...
    if (ctl->a()) {
        ...
        //controllerData.println("controllerData");
        digitalWrite(2,led ? HIGH : LOW);
        //Serial1.println("Serial1 here");
        //controllerData.println("dataController here");
        led = !led;
...
void setup() {
    Serial.begin(115200);
    Serial1.begin(9600, SERIAL_8N1, 16, 17);  // Initialize the UART1 to communicate with ESP8266
    Serial.println("ESP32 is ready to send and receive on Serial1.");
    Serial1.println("Test msg on Seiral 1");
    ...
    controllerData.begin(controllerDataBaud, SERIAL_8N1, rxd2, txd2);  // Baud rate, 8 data bits, no parity, 1 stop bit

    Serial.printf("Controller Data Serial Line established @ %d baud.\n",controllerDataBaud);
    Serial1.printf("Controller Data Serial Line established @ %d baud.\n",controllerDataBaud);
